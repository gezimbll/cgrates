...
func (aS *AccountS) accountDebit(ctx *context.Context, acnt *utils.Account, usage *decimal.Big,
	...
	attrConns, err := engine.GetConnIDs(ctx, aS.cfg.AccountSCfg().Conns[utils.MetaAttributes], cgrEv.Tenant, cgrEv.AsDataProvider(), aS.fltrS, utils.MetaAttributes)
	if err != nil {
		return nil, err
	}
	rateConns, err := engine.GetConnIDs(ctx, aS.cfg.AccountSCfg().Conns[utils.MetaRates], cgrEv.Tenant, cgrEv.AsDataProvider(), aS.fltrS, utils.MetaRates)
	if err != nil {
		return nil, err
	}

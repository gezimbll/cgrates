...
func (sS *SessionS) Shutdown() (err error) {
	replConns, _ := engine.GetConnIDs(context.TODO(), sS.cfg.SessionSCfg().Conns[utils.MetaReplication], utils.MetaAny, nil, sS.fltrS)
	if len(replConns) == 0 {
		var hasErr bool
...
func (sS *SessionS) forceSTerminate(ctx *context.Context, s *Session, extraUsage time.Duration, tUsage, lastUsed *time.Duration) (err error) {
	...
	replConns, _ := engine.GetConnIDs(ctx, sS.cfg.SessionSCfg().Conns[utils.MetaReplication], utils.MetaAny, nil, sS.fltrS)
...
func (sS *SessionS) updateSession(ctx *context.Context, s *Session, updtEv, opts engine.MapEvent,
	...
	defer func() {
		replConns, _ := engine.GetConnIDs(ctx, sS.cfg.SessionSCfg().Conns[utils.MetaReplication], utils.MetaAny, updtEv, sS.fltrS)
		sS.replicateSessions(ctx, s.ID, false, replConns)
	}()
	s.lk.Lock()
	defer s.lk.Unlock()

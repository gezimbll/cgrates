...
func TestForceSTerminatorPostCDRs(t *testing.T) {
	log.SetOutput(io.Discard)
	cfg := config.NewDefaultCGRConfig()
	cfg.SessionSCfg().Opts.Conns[utils.MetaCDRs] = []*config.DynamicStringSliceOpt{
...
func TestForceSTerminatorReleaseSession(t *testing.T) {
	log.SetOutput(io.Discard)
	cfg := config.NewDefaultCGRConfig()
	cfg.SessionSCfg().Opts.Conns[utils.MetaResources] = []*config.DynamicStringSliceOpt{
...
dm := engine.NewDataManager(dbCM, cfg.CacheCfg(), connMgr)
	...
	cfg.SessionSCfg().Opts.Conns[utils.MetaChargers] = []*config.DynamicStringSliceOpt{
...
func TestBiRPCv1AuthorizeEventWithDigest(t *testing.T) {
	...
	chanInternal <- clnt
	cfg := config.NewDefaultCGRConfig()
	cfg.SessionSCfg().Opts.Conns[utils.MetaAttributes] = []*config.DynamicStringSliceOpt{
	...
	cfg.SessionSCfg().Opts.Conns[utils.MetaRoutes] = []*config.DynamicStringSliceOpt{
...
func TestBiRPCv1ForceDisconnect(t *testing.T) {
	...
	clntConn <- testMk
	sessions.cfg.SessionSCfg().Opts.Conns[utils.MetaResources] = []*config.DynamicStringSliceOpt{
		{Values: []string{utils.ConcatenatedKey(utils.MetaInternal, utils.MetaResources)}},
	}

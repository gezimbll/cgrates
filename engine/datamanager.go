...
func (wb *WriteBuff) AddInMap(key string, op func() error) {
	wb.mu.Lock()
	defer wb.mu.Unlock()
	wb.ops[key] = op
}
...
func (dm *DataManager) SetThresholdProfile(ctx *context.Context, th *ThresholdProfile, withIndex bool) (err error) {
	...
	if dm.async {
		if errCh := Cache.Set(ctx, utils.CacheThresholdProfiles, utils.ConcatenatedKey(th.Tenant, th.ID), th, nil,
			cacheCommit(utils.NonTransactional), utils.NonTransactional); errCh != nil {
			return errCh
		}
...
func (dm *DataManager) GetResourceProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	if err != nil {
		if itm := dm.cfg.DataDbCfg().Items[utils.MetaResourceProfile]; err == utils.ErrNotFound && itm.Remote {
			if err = dm.connMgr.Call(ctx, dm.cfg.DataDbCfg().RmtConns,
...
func (dm *DataManager) SetResourceProfile(ctx *context.Context, rp *ResourceProfile, withIndex bool) (err error) {
	...
	if itm := dm.cfg.DataDbCfg().Items[utils.MetaResourceProfile]; itm.Replicate {
		if err = replicate(ctx, dm.connMgr, dm.cfg.DataDbCfg().RplConns,
			dm.cfg.DataDbCfg().RplFiltered,

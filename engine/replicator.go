...
func (r *replicator) replicateAndRestore(ctx *context.Context, key string, objType, objID, method string, args any) error {
	...
	if err != nil && failedPath != "" {
		...
		if wErr := task.WriteToFile(ctx, failedPath); wErr != nil {
...
func (r *replicator) flush() {
	...
	for key, data := range pending {
		if err := r.replicateAndRestore(context.Background(), key, data.objType, data.objID, data.method, data.args); err != nil {
			utils.Logger.Warning(fmt.Sprintf(

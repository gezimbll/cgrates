...
func (dDP *DynamicDP) fieldAsInterface(fldPath []string) (val any, err error) {
	...
	case utils.MetaAccounts:
		...
		if err = connMgr.Call(dDP.ctx, acctConns, utils.AccountSv1GetAccount,
	...
	case utils.MetaResources:
		...
		if err := connMgr.Call(dDP.ctx, resConns, utils.ResourceSv1GetResourceWithConfig,
	...
	case utils.MetaStats:
		...
		if err := connMgr.Call(dDP.ctx, statConns, utils.StatSv1GetQueueDecimalMetrics,
	...
	case utils.MetaTrends:
		...
		if err := connMgr.Call(dDP.ctx, trendConns, utils.TrendSv1GetTrendSummary, &utils.TenantIDWithAPIOpts{TenantID: &utils.TenantID{Tenant: dDP.tenant, ID: fldPath[1]}}, &trendSum); err != nil {
			return nil, err
		}
	...
	case utils.MetaRankings:
		...
		if err := connMgr.Call(dDP.ctx, rnkConns, utils.RankingSv1GetRankingSummary, &utils.TenantIDWithAPIOpts{TenantID: &utils.TenantID{Tenant: dDP.tenant, ID: fldPath[1]}}, &rankingSum); err != nil {
			return nil, err
		}

...
func TestApierCfgloadFromJsonCfg(t *testing.T) {
	...
	expected := &AdminSCfg{
		Enabled: false,
		Conns: map[string][]*DynamicStringSliceOpt{
			utils.MetaCaches:     {{Values: []string{utils.ConcatenatedKey(utils.MetaInternal, utils.MetaCaches)}}, {Values: []string{utils.ConcatenatedKey(utils.MetaInternal, utils.MetaCaches), "*conn1"}}},
...
func TestApierCfgAsMapInterface2(t *testing.T) {
	...
	expectedMap := map[string]any{
		...
		utils.ConnsCfg: map[string][]*DynamicStringSliceOpt{
			utils.MetaAttributes: {{Values: []string{utils.MetaInternal, "*conn1"}}},
			utils.MetaEEs:        {{Values: []string{utils.MetaInternal, "*conn1"}}},
			utils.MetaCaches:     {{Values: []string{utils.MetaInternal}}, {Values: []string{utils.MetaInternal, "*conn1"}}},
			utils.MetaActions:    {{Values: []string{utils.MetaInternal, "*conn1"}}},
		},
	}

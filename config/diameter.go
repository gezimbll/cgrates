...
func (da *DiameterAgentCfg) loadFromJSONCfg(jc *DiameterAgentJsonCfg, separator string) (err error) {
	...
	if jc.Listeners != nil {
		...
		for _, listnr := range *jc.Listeners {
			...
			if listnr.Listen != nil {
				ls.Listen = *listnr.Listen
			}
			if listnr.ListenNet != nil {
				ls.ListenNet = *listnr.ListenNet
			}
...
func (lstn *DiameterListener) AsMapInterface() map[string]any {
	return map[string]any{
		utils.ListenNetCfg: lstn.ListenNet,
		utils.ListenCfg:    lstn.Listen,
	}

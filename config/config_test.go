...
func TestV1GetConfigStats(t *testing.T) {
	...
	expected := map[string]any{
		STATS_JSON: map[string]any{
			...
			utils.EEsConnsCfg:          []string{},
			utils.OnlineStatExportsCfg: []string{},
		},
	}
	cfgCgr := NewDefaultCGRConfig()
...
func TestV1GetConfigAsJSONStatS(t *testing.T) {
	var reply string
	expected := `{"stats":{"ees_conns":[],"enabled":false,"indexed_selects":true,"nested_fields":false,"online_stat_exports":[],"opts":{"*profileIDs":[],"*profileIgnoreFilters":false},"prefix_indexed_fields":[],"store_interval":"","store_uncompressed_limit":0,"suffix_indexed_fields":[],"thresholds_conns":[]}}`
	cgrCfg := NewDefaultCGRConfig()

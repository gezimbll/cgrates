...
func TestSetCfgInDb(t *testing.T) {
	...
	expected := &AttributeSJsonCfg{
		Enabled: utils.BoolPointer(false),
		Conns: map[string][]*DynamicStringSliceOpt{
			utils.MetaResources: {{Values: []string{"*internal"}}, {Values: []string{"*localhost"}}},
			utils.MetaStats:     {{Values: []string{"*internal"}}, {Values: []string{"*localhost"}}},
			utils.MetaAccounts:  {{Values: []string{"*internal"}}, {Values: []string{"*localhost"}}},
		},
		Indexed_selects:          utils.BoolPointer(false),
		String_indexed_fields:    &[]string{"field2"},
		Suffix_indexed_fields:    &[]string{"field2"},

...
func TestTrendSLoadFromJSONCfgEesConns(t *testing.T) {
	...
	// Loading appends new conn entries to existing ones
	if len(trendCfg.Conns[utils.MetaEEs]) != 2 {
		t.Errorf("Expected EEs conns slice length to be 2, but got: %d", len(trendCfg.Conns[utils.MetaEEs]))
	}
	expectedFirst := []string{"old_conn1", "old_conn2"}
	expectedSecond := []string{"new_conn1", "new_conn2"}
	...
	if !reflect.DeepEqual(trendCfg.Conns[utils.MetaEEs][1].Values, expectedSecond) {
		t.Errorf("Expected second entry %v, but got: %v", expectedSecond, trendCfg.Conns[utils.MetaEEs][1].Values)
	}
}

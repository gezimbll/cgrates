...
func (da *DiameterAgent) ListenAndServe(stopChan <-chan struct{}) (err error) {
	...
	for _, lstnrCfg := range da.cgrCfg.DiameterAgentCfg().Listeners {
		utils.Logger.Info(fmt.Sprintf("<%s> Start listening on <%s>", utils.DiameterAgent, lstnrCfg.Listen))
		srv := &diam.Server{
			Network: lstnrCfg.ListenNet,
			Addr:    lstnrCfg.Listen,
			Handler: dSM,
			Dict:    nil,
		}
		...
		if err != nil {
			utils.Logger.Err(fmt.Sprintf("<%s> failed to bind listener %s: %v", utils.DiameterAgent, lstnrCfg.Listen, err))

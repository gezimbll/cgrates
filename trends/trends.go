...
func (t *TrendS) processThresholds(trnd *utils.Trend) (err error) {
	...
	threshConns, err := engine.GetConnIDs(context.TODO(), t.cfg.TrendSCfg().Conns[utils.MetaThresholds], trnd.Tenant, trnd, t.fltrS)
...
func (t *TrendS) processEEs(trnd *utils.Trend) (err error) {
	...
	eesConns := engine.GetConnIDs(t.cfg.TrendSCfg().Conns[utils.MetaEEs])
	if len(eesConns) == 0 {
		return
	}
	opts := map[string]any{

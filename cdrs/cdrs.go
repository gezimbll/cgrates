...
func (cdrS *CDRServer) rateSCostForEvent(ctx *context.Context, cgrEv *utils.CGREvent) (err error) {
	...
	var conns []string
	conns, err = engine.GetConnIDs(ctx, cdrS.cfg.CdrsCfg().Conns[utils.MetaInternal], cgrEv.Tenant, cgrEv.AsDataProvider(), cdrS.fltrS)
	if err != nil {
		return
	}
	if err = cdrS.connMgr.Call(ctx, conns,
...
func (cdrS *CDRServer) accountSRefundCharges(ctx *context.Context, tnt string, eChrgs *utils.EventCharges,
	...
	connIDs, err := engine.GetConnIDs(ctx, cdrS.cfg.CdrsCfg().Conns[utils.MetaAccounts], tnt, nil, cdrS.fltrS)
	if err != nil {
		return
	}

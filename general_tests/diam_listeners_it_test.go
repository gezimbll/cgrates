	...
	content := `
{
	...
	"diameter_agent": {
		"enabled": true,
		"listeners": [
        {"listen": "127.0.0.1:3868", "network": "tcp"},
        {"listen": "127.0.0.2:3869", "network": "tcp"}
       ],
...
`
	...
	t.Run("TestMultipleListeners", func(t *testing.T) {
		...
		for i, lCfg := range listeners {
			...
			cli, err := agents.NewDiameterClient(lCfg.Listen, "INTEGRATION_TEST_CLIENT",
				...
				cfg.DiameterAgentCfg().DictionariesPath, lCfg.ListenNet)

			if err != nil {
				t.Fatalf("Failed to create client for %s: %v", lCfg.Listen, err)
			}
			...
			if err := cli.SendMessage(m); err != nil {
				t.Errorf("Failed to send message to listener %s: %v", lCfg.Listen, err)
				continue
			}

			reply := cli.ReceivedMessage(time.Second)
			if reply == nil {
				t.Errorf("No reply received from listener %s", lCfg.Listen)
				continue
			}
			...
			if len(avps) == 0 {
				t.Errorf("Origin-Host AVP missing in reply from %s", lCfg.Listen)
				continue
			}

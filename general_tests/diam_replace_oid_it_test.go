...
cgrates.org,DEFAULT,,,*default,*none,0`,
	...
	diamClient, err := agents.NewDiameterClient(cfg.DiameterAgentCfg().Listeners[0].Listen, "localhost",
		...
		cfg.DiameterAgentCfg().DictionariesPath, cfg.DiameterAgentCfg().Listeners[0].ListenNet)
	if err != nil {
		t.Fatal(err)
	}
